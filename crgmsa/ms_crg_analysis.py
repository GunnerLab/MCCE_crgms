...
try:
    ...
    try:
        if Path(__file__).parent.name == "crgmsa":
            from crgmsa import ms_analysis as msa
...
class WeightedCorr:
    def __init__(
        ...
        if no_full_df:
            ...
            self.x, self.y, self.w = (pd.to_numeric(xyw_df[c], errors="coerce").values for c in xyw_df.columns)
            self.df = None
        else:
    ...
    def __call__(self, method: str = "pearson") -> Union[float, pd.DataFrame]:
        ...
        if method not in ["pearson", "spearman"]:
            logger.error("ValueError: correlation method not in ['pearson','spearman']")
            sys.exit(2)
        ...
        if self.df is None:
            ...
            for i, x in enumerate(self.df.columns):
                for j, y in enumerate(self.df.columns):
                    if i >= j:
                        out.loc[x, y] = cor(
                            x=pd.to_numeric(self.df[x], errors="coerce"), y=pd.to_numeric(self.df[y], errors="coerce")
                        )
...
def plots_unique_crg_histogram(charge_ms_info: tuple, background_charge: float, save_dir: Path):
    """
...
def convert_ms_crg(crg_lst: list, d: dict):
    out = [[y[0], y[1], [convert_ms_crg(x, d) if isinstance(x, list) else d.get(x, x) for x in y[2]]] for y in crg_lst]
    return out


def find_uniq_crgms_count_order(
    crg_list_ms: list, E_range: Tuple[float, float] = None
...
def correlation_data_parsing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info(("Number of residues that change the protonation state: ", f"{len(all_crg_count_std.columns)-1}"))
...
def get_topN_crg_microstates(
    ...
    ms_free_res_df = msa.free_residues_df(mc.free_residues, conformers, colname="Residue")
    background_charge, fixed_res_crg_df = fixed_residues(mc.fixed_iconfs, conformers, res_of_interest)
    ...
    all_res_crg_count_df = concat_crgms_dfs(
...
def crgmsa_parser() -> ArgumentParser:
    ...
    p.add_argument("-run_dir", type=str, default=".", help="titration point; default: %(default)s.")
...
def crgmsa_cli(argv=None):
    ...
    if args.top_n is not None:
        get_topN_crg_microstates(args.run_dir, ph_pt=args.ph_pt, top_n=args.top_n, res_of_interest=args.res_of_interest)
    else:
